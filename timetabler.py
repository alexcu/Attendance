...
#This first one checks whether the tutors have enough availabilities for their classes.
availabilitieserror = DataFrame(Tutor = String[], Availabilities = Int64[], Classes = Int64[])
for i in 1:length(availabilities[:Tutor])
tutor = availabilities[:Tutor][i]
classes = sum(subgroupteachers[i][:Repeats])
navailabilities = sum(availabilities[i,k] for k=2:(ncol(availabilities)-1))
push!(availabilitieserror,[tutor,navailabilities,classes])
end
for i in 1:length(availabilitieserror[:Tutor])
classes = availabilitieserror[:Classes][i]
navailabilities = availabilitieserror[:Availabilities][i]
if classes > navailabilities
error("$(availabilitieserror[:Tutor][i]) does not have sufficient availability slots for the number of allocated classes and repeats.")
...
for m in TEACHERS:
    for j in SUBJECTS:
        if j in TEACHERMAPPING[m]:
            ...
            model+= lpSum(subject_vars[(j,k,m)] for k in TIMES) == 1
        else:
        ...
        for k in TIMES:
            for m in TEACHERS:
                if assign_vars[(i,j,k,m)].varValue == 1:
                    print((i,j,k,m))
for j in TEACHERS:
    print(daysforteacherssum[(j)].varValue)
